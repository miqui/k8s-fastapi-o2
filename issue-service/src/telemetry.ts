...
// Pushes metrics as OTLP to the same OTel Collector message-service uses
// (OTEL_METRICS_URL, see k8s/configmap.yaml), tagged with service.name = "issue-service"
// so Prometheus/Grafana can tell the two APIs' series apart.
...
const meter = meterProvider.getMeter("issue-service");

// No `unit` option on "_ms"-suffixed instruments below - see message-service's
// src/telemetry.ts for why (double-suffixing via the OTel Collector's Prometheus
// exporter).

// Labeled by the schema's own operation type and root field (see metricsPlugin), deliberately
// NOT by the client-supplied operationName: it's unbounded (any client can invent names),
// which would let callers grow the series count in the collector and in OpenObserve at will.
...
// Prisma's own connection-pool and query-timing stats (prisma.$metrics.json(), needs
// the "metrics" preview feature in prisma/schema.prisma), sampled once per collection
// tick via a batch callback rather than one client.$metrics.json() round trip per gauge.
